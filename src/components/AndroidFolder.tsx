...
export const AndroidFolder: React.FC<AndroidFolderProps> = ({ 
  ...
  simulatedOffset 
    ...
    const c_top = windowSize.h - b_offset - 48; 
    const c_left = isMain ? windowSize.w - 88 : 40;

    return (
      <div 
        style={isExpanded ? { top: 0, left: 0, width: '100%', height: '100%', borderRadius: 0, zIndex: 500 } : { top: `${c_top}px`, left: `${c_left}px`, width: '48px', height: '48px', borderRadius: '0.75rem', zIndex: 100 - index }} 
        className={`fixed shadow-2xl overflow-hidden transition-all duration-300 ${isExpanded ? 'bg-white/95 backdrop-blur-3xl' : `${data.color} cursor-pointer border-t border-white/20`} flex items-center justify-center text-white`} 
        ...
        { !isMain && (
             <div className="absolute bottom-3 left-1/2 -translate-x-1/2 opacity-50">
                 {activeView === fId ? <ChevronDown size={16}/> : <ChevronUp size={16}/>}
             </div>
        )}
      </div>
    );
