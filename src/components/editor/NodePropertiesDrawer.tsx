...
export const NodePropertiesDrawer: React.FC<Props> = ({
  node,
  isOpen,
  onClose,
  onUpdate,
      ...
      <SheetContent side="bottom" className="h-[80vh] sm:h-[60vh] rounded-t-[2rem] border-t-4 border-primary/20">
        <SheetHeader className="flex flex-row items-center justify-between pb-4 border-b">
          <div>
            ...
            <SheetDescription className="text-xs uppercase tracking-widest font-bold opacity-70">
              Type: {node.type}
            </SheetDescription>
          </div>
          ...
          <div className="space-y-6 pb-20">
            ...
            <div className="space-y-2">
              ...
              <Textarea 
                ...
                className="font-code text-xs h-32 rounded-xl border-2"
                value={node.payloadSchema || ''}
                onChange={(e) => onUpdate({ payloadSchema: e.target.value })}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea 
                ...
                className="font-code text-xs h-48 rounded-xl border-2"
                value={node.customCode || ''}
                onChange={(e) => onUpdate({ customCode: e.target.value })}
              />
              <p className="text-[10px] text-muted-foreground italic px-1">Edge deployment is automatic.</p>
            </div>
          </div>
        </ScrollArea>
        ...
        <SheetFooter className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background via-background to-transparent border-t flex-row gap-4">
          <Button variant="destructive" className="flex-1 h-12 rounded-xl shadow-lg" onClick={onDelete}>
            <Trash2 className="w-4 h-4 mr-2" /> Delete
          </Button>
          <Button className="flex-1 h-12 rounded-xl shadow-lg" onClick={onClose}>
